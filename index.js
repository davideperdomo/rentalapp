...
const CRIMEICON = 'https://i.imgur.com/x3yfpB0.png'
const ATMICON = 'http://www.cityofukiah.com/NewWeb/wp-content/themes/deepblue/images/icon-dollar-small.png'
const FIRESTICON = 'https://i.imgur.com/3Y3Wowa.png'
const ARTICON = "https://i.imgur.com/72pekwx.png"
const MUSICON = "https://i.imgur.com/PLGsudw.png"

const NYUICON = "https://i.imgur.com/gvltPae.png"
const ECOICON = "https://i.imgur.com/M1UDxX0.png"
const SAFEICON = "https://i.imgur.com/SKwtNnA.png"
...
function inputRadio(){
    ...
    price = (document.getElementById("inmaxprice").value);
}
...
/*Police precinct in ney york data set based in hacker 7 code
--------------------------------------------------------------------------------
*/
var polygons = []
var policeprePath = [];
...
function minCrimes(){
  min=HOUSESDATA[0]
  for(i=1;i<HOUSESDATA.length;i++){
    if(HOUSESDATA[i][6]<min[6]){
      min=HOUSESDATA[i]
    }
  }
...
function myMap() {
    ...
    /*var markerCluster = new MarkerClusterer(map, artMarkers,
...
function addMarkerHouse(marker){
    ...
    var pos = new google.maps.LatLng(marker[IDX_LAT], marker[IDX_LON]);
    var image;
    if(marker[5]=="Gramercy"){image = ECOICON;}
    else if(marker[5]=="Stuyvesant Town-Cooper Village"){image = SAFEICON;}
    else{image = HOUSEICON;}
    gmarker = new google.maps.Marker({
        title: title,
        position: pos,
        category: category,
    ...
    var infowindow = new google.maps.InfoWindow();
    google.maps.event.addListener(gmarker, 'click', function () {
        infowindow.setContent("<br><h2>"+marker[2]+" "+this.title+"</h2></br><br>Neighbor: "+marker[5]
          +"</br><br>Rent aprox. price: "+marker[7]+"</br><br>Num crimes: "+marker[6]+"</br><br>Distance to NYU: "+(marker[4]*100)+" kms</br><br><a href=\"https://www.zillow.com/homes/for_rent/"+marker[2]+" "+this.title+"-,-NY_rb/\" target=\"_blank\">View in Zillow</br>")
        infowindow.open(map, this);

        var newdiv = document.createElement("div");
        newdiv.className = "optiondiv";
        //newdiv.append(text) 
        newdiv.innerHTML = "<br><b>"+marker[2]+" "+this.title+"</b><br>Neighbor: "+marker[5]
          +"<br>Rent aprox. price: "+marker[7]+"<br>Num crimes: "+marker[6]+"<br>Distance to NYU: "+(marker[4]*100)+" kms<br><a href=\"https://www.zillow.com/homes/for_rent/"+marker[2]+" "+this.title+"-,-New-York-city_rb/\" target=\"_blank\">View in Zillow";
...
function addMarkersHouse(opt){
  for (i=0; i < HOUSESDATA.length; i++){
        if (radio>HOUSESDATA[i][4] && price>HOUSESDATA[i][7]){
...
function addMarkerRent(marker){
    ...
    var image = "https://static.vesselfinder.net/images/empty20.png"
    gmarker = new google.maps.Marker({
        title: title,
        position: pos,
        map: map,
...
$(document).ready(function() {
  ...
  $("#safe").on("click",safeMap);
  $("#cheap").on("click",cheapMap);
